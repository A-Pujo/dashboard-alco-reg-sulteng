...
export default function DashboardKinerjaFiskal() {
  ...
  const aggregateAPBNData = useCallback((data) => {
    ...
    const latestData = data.filter(item => new Date(item.tgl_cutoff).getTime() === latestDate.getTime());
  ...
  const aggregateAPBNIaccountData = useCallback((rawData) => {
    const aggregated = {
      pendapatan: {},
      belanja: {},
    ...
    const latestData = rawData.filter(item => new Date(item.tgl_cutoff).getTime() === latestDate.getTime());
    ...
    aggregated.surplusDefisitRealisasi = aggregated.totalPendapatanRealisasi - aggregated.totalBelanjaRealisasi;
    aggregated.surplusDefisitPagu = aggregated.totalPendapatanPagu - aggregated.totalBelanjaPagu;
  ...
  const prepareChartData = useCallback((apbnRawData, apbdRawData) => {
    ...
    monthlyDataMap.forEach((monthData, yearMonth) => {
      ...
      if (monthData.apbnMonthlyItems.length > 0) {
        const latestAPBNDateForMonth = monthData.apbnMonthlyItems.reduce((maxDate, item) => {
          const currentDate = new Date(item.tgl_cutoff);
          return currentDate > maxDate ? currentDate : maxDate;
        }, new Date(0));

        const latestAPBNItemsForMonth = monthData.apbnMonthlyItems.filter(item =>
          new Date(item.tgl_cutoff).getTime() === latestAPBNDateForMonth.getTime()
        );
      ...
      if (monthData.apbdMonthlyItems.length > 0) {
        const latestAPBDDateForMonth = monthData.apbdMonthlyItems.reduce((maxDate, item) => {
          const currentDate = new Date(item.tgl_cutoff);
          return currentDate > maxDate ? currentDate : maxDate;
        }, new Date(0));

        const latestAPBDItemsForMonth = monthData.apbdMonthlyItems.filter(item =>
          new Date(item.tgl_cutoff).getTime() === latestAPBDDateForMonth.getTime()
        );
  ...
  const fetchFiskalData = useCallback(async (filters) => {
    ...
    try {
      ...
      const { data: apbdCurrentRawData, error: apbdError } = await supabase
        .from('fiskal_pemda')
        .select('tgl_cutoff, nama_pemda, pendapatan, belanja, pembiayaan, SILPA') // Added nama_pemda
        .gte('tgl_cutoff', waktu_awal)
        .lte('tgl_cutoff', waktu_akhir)
        .order('tgl_cutoff', { ascending: true }); // Order by date
      ...
      const iAccountAggregatedData = aggregateAPBNIaccountData(apbnCurrentRawData);
      setApbnIaccountData(iAccountAggregatedData);

      // NEW: Aggregate data for the APBD per-Pemda table
      const pemdaTableAggregatedData = aggregateAPBDTableData(apbdCurrentRawData);
      ...
      const apbnPendapatanYoY = calculateYoYChange(apbnCurrentSummary.totalPendapatanRealisasi, apbnPrevSummary.totalPendapatanRealisasi);
      const apbnBelanjaYoY = calculateYoYChange(apbnCurrentSummary.totalBelanjaRealisasi, apbnPrevSummary.totalBelanjaRealisasi);
      ...
      setFiskalData({
        ...
        apbn: {
          ...
          surplusDefisit: apbnCurrentSummary.totalPendapatanRealisasi - apbnCurrentSummary.totalBelanjaRealisasi,
        ...
        apbd: {
          ...
          surplusDefisit: apbdCurrentSummary.totalPendapatan - apbdCurrentSummary.totalBelanja + apbdCurrentSummary.totalPembiayaan,
        ...
        totalKonsolidasi: {
          totalPendapatan: apbnCurrentSummary.totalPendapatanRealisasi + apbdCurrentSummary.totalPendapatan,
          totalBelanja: apbnCurrentSummary.totalBelanjaRealisasi + apbdCurrentSummary.totalBelanja,
          surplusDefisit: (apbnCurrentSummary.totalPendapatanRealisasi - apbnCurrentSummary.totalBelanjaRealisasi) +
      ...
      const preparedChartData = prepareChartData(apbnCurrentRawData, apbdCurrentRawData);
  ...
  }, [aggregateAPBNData, aggregateAPBDData, aggregateAPBNIaccountData, aggregateAPBDTableData, prepareChartData, apbnPendapatanKompAng, apbnBelanjaKompAng]); // Added aggregateAPBDTableData to dependencies
      ...
      {fiskalData && apbnIaccountData && !isLoading && !error && (
        <>
          ...
          <div className='flex justify-between items-center mb-4'>
            ...
            <Link href={`/fiskal/belanja-negara`} className="btn btn-xs btn-primary rounded-full">
              Rincian Belanja
            </Link>
          </div>
          ...
          <section className="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 className="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-4">APBN I-Account (Target vs Realisasi)</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Penerimaan (Pendapatan Negara) */}
              <div className="border border-gray-200 rounded-md p-3">
                <h3 className="text-md font-bold text-gray-700 mb-3">Penerimaan (Pendapatan Negara)</h3>
                ...
                {apbnPendapatanKompAng.map(komp => (
                  <div key={komp} className="flex justify-between items-center text-sm py-1">
                    <span>{componentDisplayNames[komp] || komp}</span>
                    <span className="text-right">
                      {formatLargeNumber(apbnIaccountData.pendapatan[komp]?.pagu_target || 0, 2)} / {formatLargeNumber(apbnIaccountData.pendapatan[komp]?.realisasi || 0, 2)}
                    </span>
                  </div>
                ))}
                <div className="flex justify-between items-center text-md font-bold text-gray-800 border-t pt-2 mt-2">
                  ...
                  <span className="text-right">
                    {formatLargeNumber(apbnIaccountData.totalPendapatanPagu, 2)} / {formatLargeNumber(apbnIaccountData.totalPendapatanRealisasi, 2)}
                  </span>
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-md p-3">
                <h3 className="text-md font-bold text-gray-700 mb-3">Pengeluaran (Belanja Negara)</h3>
                <div className="flex justify-between items-center text-sm font-semibold text-gray-600 border-b pb-1 mb-2">
                  <span>Komponen</span>
                  <span className="text-right">Target / Realisasi</span>
                </div>
                {apbnBelanjaKompAng.map(komp => (
                  <div key={komp} className="flex justify-between items-center text-sm py-1">
                    <span>{componentDisplayNames[komp] || komp}</span>
                    <span className="text-right">
                      {formatLargeNumber(apbnIaccountData.belanja[komp]?.pagu_target || 0, 2)} / {formatLargeNumber(apbnIaccountData.belanja[komp]?.realisasi || 0, 2)}
                    </span>
                  </div>
                ))}
                <div className="flex justify-between items-center text-md font-bold text-gray-800 border-t pt-2 mt-2">
                  ...
                  <span className="text-right">
                    {formatLargeNumber(apbnIaccountData.totalBelanjaPagu, 2)} / {formatLargeNumber(apbnIaccountData.totalBelanjaRealisasi, 2)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 p-3 bg-base-200 rounded-md flex justify-between items-center text-lg font-bold text-gray-800">
              ...
              <span className="text-right">
                {formatLargeNumber(apbnIaccountData.surplusDefisitPagu, 2)} / {formatLargeNumber(apbnIaccountData.surplusDefisitRealisasi, 2)}
              </span>
            </div>
          </section>
          ...
          {apbdPemdaTableData && apbdPemdaTableData.length > 0 ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  <tr>
                    ...
                    {apbdPemdaTableData.map((pemda) => (
                      <td key={`${pemda.nama_pemda}-pendapatan`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-right">
                  ...
                  <tr>
                    ...
                    {apbdPemdaTableData.map((pemda) => (
                      <td key={`${pemda.nama_pemda}-belanja`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-right">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md text-center text-gray-500 mb-6">
              Tidak ada data APBD per pemerintah daerah yang tersedia untuk periode ini.
            </div>
          )}
          ...
          <section className="bg-white p-4 rounded-lg shadow-md mb-6">
            ...
            <ResponsiveContainer width="100%" height={400} className={`text-xs`}>
              ...
              >
                ...
                <YAxis tickFormatter={formatCurrencyAxis} tick={{ fontSize: 10 }} />
