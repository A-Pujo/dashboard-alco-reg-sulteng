...
export default function TableMakro() {
  ...
  const fetchVariableData = async () => {
    ...
    try {
      const fetchPromises = tahunList.map(async (yearInfo) => {
        ...
        // console.log(`https://webapi.bps.go.id/v1/api/list/model/data/lang/ind/domain/${selectedDomain}/var/${selectedVariable}/th/${yearInfo.th_id}/key/${BPSKey}`)
  ...
  return (
    <main className="flex-1 overflow-y-auto p-4 md:p-8 mt-16 md:mt-12">
      <div className='flex border-b border-gray-300 py-2 mb-4'>
        <h2 className='flex-1 text-xl font-bold'>Data Makro Ekonomi <sup className='text-xs font-light'>BPS Table</sup></h2>
      </div>
                  ...
                  <tbody>
                    {fetchedData.vervar.map((vervar, i) => (
                      fetchedData.turvar.map((turvar, j) => (
                        <tr key={`${i}-${j}`}>
                          <th className='bg-white' rowSpan={1}>{vervar.label}</th>
